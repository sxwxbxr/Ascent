...
Ascent is an Android-based gym/fitness app with a browser dashboard, built as a private project for 2-3 users (the owner plus 1-2 training partners). **M0–M2 are live in production** (backend with auth/CRUD/sync, 1,324 exercises + media in R2); **M3 (Android core) is implemented and bundle-verified but has never run on a device**. A GitHub Actions workflow (`android-apk.yml`) builds the sideloadable beta APK. Next: device testing, M4 (sync client), M5 (web dashboard). **Read `PROJEKTSTATUS.md` first** — it holds the current state, known pitfalls, and open points.
...
- **`apps/mobile`** — Expo (SDK 57) + NativeWind v4, which requires **Tailwind 3.4.x** (not v4 — the two apps intentionally differ). expo-router with a session guard (`Stack.Protected` in `app/_layout.tsx` — new routes must be added to the guarded block). Offline-first: local SQLite via `drizzle-orm/expo-sqlite` reusing the **shared schema 1:1**; migrations generated by `pnpm --filter @ascent/mobile db:generate` (drizzle-kit driver `expo`, bundled via babel `inline-import` + metro `sql` sourceExt). Auth: Better-Auth Expo client (SecureStore-cached session works offline). UUIDs via `src/lib/ids.ts` (expo-crypto — Hermes lacks `crypto.randomUUID`). `useLiveQuery` pitfalls: needs explicit deps for parameterized queries, and only reacts to the FROM base table (not joins). Verification without a device: `typecheck` + `expo export --platform android` (Metro bundle smoke).
