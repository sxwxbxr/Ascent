...
- **`apps/api`** — Cloudflare Worker (Hono), bindings `DB` (D1) and `MEDIA` (R2) defined in `wrangler.jsonc`. Migrations are generated from the shared schema via drizzle-kit into `apps/api/drizzle/`. Feature flags live in the `feature_flags` table (seed: `seed/feature_flags.sql`); `/entitlements` resolves them per tier — features must never be gated by hardcoded conditions.
